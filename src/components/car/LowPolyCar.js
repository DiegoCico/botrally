...
// --- Wheels (unchanged diagnostic version) ---
...
export function buildLowPolyCar({
  ...
  trimColor   = 0x3b4556,
  glassColor  = 0x9fb4c6,
  lightColor  = 0x9fe7ff,   // headlights hue
  tireColor   = 0x22262b,
  rimColor    = 0xcfd6dd,
  scale       = 1.0,
  spoilerType = 'none',
  ...
  hood.position.set(1.0, 1.0, 0); hood.rotation.z = THREE.MathUtils.degToRad(-5);
  add(hood, 'body');

  const roof = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.55, 1.5), materials.body);
  roof.position.set(-0.4, 1.55, 0); roof.rotation.z = THREE.MathUtils.degToRad(5);
  add(roof, 'body');

  const windshield = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.45, 1.5), materials.glass);
  windshield.position.set(0.45, 1.45, 0); windshield.rotation.z = THREE.MathUtils.degToRad(-18);
  add(windshield, 'glass');

  const rearGlass = new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.42, 1.3), materials.glass);
  rearGlass.position.set(-1.2, 1.5, 0); rearGlass.rotation.z = THREE.MathUtils.degToRad(18);
  add(rearGlass, 'glass');
  ...
  // --- Headlights (neon) ---
  const headGeo = new THREE.BoxGeometry(0.25, 0.18, 0.25);
  function makeNeonLight(material, colorHex, haloW, haloH, spotIntensity) {
    const g = new THREE.Group();
    const lens = new THREE.Mesh(headGeo, material);
    g.add(lens);
    ...
    const halo = new THREE.Mesh(new THREE.PlaneGeometry(haloW, haloH), haloMat);
    halo.position.x += 0.06;
    g.add(halo);

    const spot = new THREE.SpotLight(colorHex, spotIntensity, 18, Math.PI / 8, 0.5, 1);
  ...
  const headL = makeNeonLight(materials.light, lightColor, 0.35, 0.28, 2.2);
  headL.position.set(2.1, 0.95, 0.6); group.add(headL);
  const headR = makeNeonLight(materials.light, lightColor, 0.35, 0.28, 2.2);
  headR.position.set(2.1, 0.95, -0.6); group.add(headR);

  // --- Tail lights (red neon) ---
  const tailL = makeNeonLight(materials.tail, 0xff0000, 0.35, 0.28, 1.2);
  tailL.position.set(-2.1, 0.95, 0.6); group.add(tailL);
  const tailR = makeNeonLight(materials.tail, 0xff0000, 0.35, 0.28, 1.2);
  tailR.position.set(-2.1, 0.95, -0.6); group.add(tailR);

  // --- Spoiler container (unchanged) ---
  ...
  return { group, materials, setBodyColor, setSpoilerType };
}
